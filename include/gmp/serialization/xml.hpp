...
inline serialization_result<void> xml_escape(std::string &out,
...
public:
  ...
  serialization_result<void> begin_array(std::size_t) { return begin("array"); }
  serialization_result<void> end_array() { return end("array"); }
  serialization_result<void> begin_object(std::size_t) { return begin("object"); }
  serialization_result<void> write_key(std::string_view key) {
    if (stack_.empty() || stack_.back().kind != "object" ||
  ...
  serialization_result<void> end_object() { return end("object"); }
...
private:
  ...
  serialization_result<void> scalar(std::string_view value,
                                    std::string_view kind,
  ...
  template <typename I>
  serialization_result<void> number(I value, std::string_view kind) {
    char buffer[32];
  ...
  serialization_result<void> end(std::string_view kind) {
    ...
    stack_.pop_back();
    serialization_result<void> status;
    if (options_.pretty && frame.count &&
...
class xml_parser {
public:
  xml_parser(std::string_view input, xml_read_options options)
  ...
  serialization_result<serialization_value> parse() {
    ...
    if (!consume("<gmp"))
      return error(serialization_errc::invalid_syntax,
                   "expected GMP XML document element");
    ...
    if (*version != "1")
      return error(serialization_errc::unsupported_type,
                   "unsupported GMP XML format version");
    skip_space();
    if (!consume(">"))
      return error(serialization_errc::invalid_syntax,
                   "expected GMP XML document close");
    skip_space();
    ...
    if (!consume("</gmp>"))
      return error(serialization_errc::invalid_syntax,
                   "expected GMP XML document end");
    skip_space();
    if (position_ != input_.size())
      return error(serialization_errc::invalid_syntax,
                   "unexpected characters after XML value");
    return value;
  }
...
private:
  ...
  declaration_attribute(std::string_view expected) {
    if (!consume(expected))
      return error(serialization_errc::invalid_syntax,
    ...
    if (position_ >= input_.size() ||
        (input_[position_] != '"' && input_[position_] != '\''))
      return error(serialization_errc::invalid_syntax,
                   "expected quoted XML declaration value");
    ...
    while (position_ < input_.size() && input_[position_] != quote) {
      ...
      if (c >= 0x80 || !is_xml_code_point(c) || input_[position_] == '<' ||
          input_[position_] == '&')
        return error(serialization_errc::invalid_syntax,
                     "invalid XML declaration value");
      ++position_;
    }
    if (position_ >= input_.size())
      return error(serialization_errc::unexpected_end,
                   "unterminated XML declaration value");
    std::string value(input_.substr(start, position_ - start));
    ++position_;
    return value;
  }
  static bool ascii_iequal(std::string_view left,
  ...
  serialization_result<void> parse_declaration() {
    ...
    if (position_ >= input_.size() || !is_space(input_[position_]))
      return error(serialization_errc::invalid_syntax,
                   "XML declaration requires a version");
    skip_space();
    ...
    for (;;) {
      ...
      if (starts("encoding")) {
        ...
        if (has_standalone)
          return error(serialization_errc::invalid_syntax,
                       "duplicate XML standalone declaration");
  ...
  serialization_result<std::string> text_until_tag() {
    ...
    if (!has_only_xml_characters(text))
      return error(serialization_errc::invalid_syntax,
                   "XML text contains a forbidden character");
    return text;
  }
  serialization_result<std::string> attribute(std::string_view expected) {
    ...
    if (position_ >= input_.size() || input_[position_] != quote)
      return error(serialization_errc::unexpected_end,
                   "unterminated XML attribute");
    ++position_;
    if (!is_valid_utf8(value))
      return error(serialization_errc::invalid_utf8,
                   "XML attribute is not valid UTF-8");
...
parse_xml(std::string_view input, xml_read_options options = {}) {
  return detail::xml_parser(input, options).parse();
}
inline serialization_result<std::string>
write_xml(const serialization_value &value, xml_write_options options = {}) {
...
template <typename T>
serialization_result<std::string>
to_xml(const T &value, serialization_options serialization = {},
...
from_xml(std::string_view input,
...
template <typename T>
std::string to_xml_or_throw(const T &value,
...
template <typename T>
T from_xml_or_throw(std::string_view input,
                    deserialization_options deserialization = {},
                    xml_read_options xml = {}) {
