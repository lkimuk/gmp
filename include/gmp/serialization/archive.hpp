...
concept serialization_writer =
    requires(A &a, bool b, std::int64_t i, std::uint64_t u, double d,
      ...
      { a.write_unsigned(u) } -> std::same_as<serialization_result<void>>;
      { a.write_floating(d) } -> std::same_as<serialization_result<void>>;
...
template <typename A>
concept serialization_reader = std::move_constructible<A> &&
                               requires(const A &a, std::size_t i) {
                                 {
                                   a.kind()
                                   } -> std::same_as<serialization_kind>;
