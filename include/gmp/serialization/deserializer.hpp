...
private:
  ...
  public:
    decoder_archive(basic_deserializer &owner, const R &reader) : owner_(owner), reader_(reader) {}

    template <typename T> serialization_result<T> decode() {
      return owner_.template decode_value<T>(reader_);
    }

    template <typename T> serialization_result<T> decode(const R &reader) {
    ...
    bool read_bool() const {
      return reader_.read_bool();
    }

    std::int64_t read_signed() const {
      return reader_.read_signed();
    }

    std::uint64_t read_unsigned() const {
      return reader_.read_unsigned();
    }

    double read_floating() const {
      return reader_.read_floating();
    }
    ...
    auto element(std::size_t index) const {
      return reader_.element(index);
    }

    auto member(std::size_t index) const {
      return reader_.member(index);
    }
  ...
  template <typename T> serialization_result<T> decode_value_impl(const Reader &r) {
    ...
    auto field = [&](std::size_t pos) {
      ...
      auto one = [&]<std::size_t J>() {
        ...
        if (D::matches(m.name)) {
          if (matched) {
            ok = false;
            error = make_serialization_error(serialization_errc::ambiguous_field,
      ...
      if (!matched && ok && options_.unknown_fields == unknown_field_policy::reject) {
        ok = false;
        error = make_serialization_error(serialization_errc::unknown_field, "unknown object field",
                                         std::string(m.name));
      }
    };
    if (r.kind() != serialization_kind::object)
...
#endif // GMP_SERIALIZATION_DESERIALIZER_HPP_
