...
struct no_serialization_default {
  friend constexpr bool operator==(no_serialization_default,
                                   no_serialization_default) = default;
};
inline constexpr no_serialization_default no_default{};
} // namespace detail
template <fixed_string Member, fixed_string Wire = Member,
          bool Transient = false, auto Default = detail::no_default,
          fixed_string... Aliases>
struct serialization_field {
  ...
  static constexpr bool has_default =
      !std::same_as<std::remove_cv_t<decltype(Default)>,
                    detail::no_serialization_default>;
  ...
  template <fixed_string Name> consteval auto alias() const {
    return serialization_field<Member, Wire, Transient, Default, Aliases...,
                               Name>{};
  }
  ...
  [[nodiscard]] static constexpr bool matches(std::string_view input) {
    return input == Wire.to_string_view() ||
           ((input == Aliases.to_string_view()) || ...);
  }
  template <typename Other>
  [[nodiscard]] static constexpr bool overlaps() {
    return Other::matches(Wire.to_string_view()) ||
           (Other::matches(Aliases.to_string_view()) || ...);
  }
  [[nodiscard]] static consteval bool names_unique() {
    constexpr std::array names{Wire.to_string_view(),
...
inline constexpr serialization_field<Member> field{};
