...
public:
  ...
  using storage =
      std::variant<null_t, bool, std::int64_t, std::uint64_t, double, std::string, array, object>;
  ...
  serialization_value(I v) noexcept
      : data_(std::is_signed_v<I> ? storage(std::int64_t(v)) : storage(std::uint64_t(v))) {}
  ...
  serialization_value(const char *v) : data_(v ? storage(std::string(v)) : storage(null_t{})) {}
  ...
  void source_offset(std::size_t v) noexcept {
    source_offset_ = v;
  }
...
#endif // GMP_SERIALIZATION_VALUE_HPP_
