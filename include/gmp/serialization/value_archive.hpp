...
public:
  ...
  serialization_result<void> begin_object(std::size_t size) {
    ...
    value.reserve(size);
    return begin(serialization_kind::object,
                 serialization_value(std::move(value)));
  }
...
private:
  ...
  serialization_result<void> put(serialization_value v) {
    ...
    f.value.get<serialization_value::object>().emplace_back(std::move(f.key),
                                                            std::move(v));
    f.key.clear();
    f.has_key = false;
    return {};
  }
  serialization_result<void> begin(serialization_kind kind,
                                   serialization_value v) {
    if (finished_)
      return make_serialization_error(serialization_errc::custom_error,
                                      "value archive is already finished");
    auto status = validate_value_position();
  ...
  serialization_result<void> validate_value_position() const {
    ...
    if (stack_.back().kind == serialization_kind::object &&
        !stack_.back().has_key)
      return make_serialization_error(serialization_errc::custom_error,
...
public:
  ...
  bool read_bool() const { return value_->get<bool>(); }
  std::int64_t read_signed() const { return value_->get<std::int64_t>(); }
  std::uint64_t read_unsigned() const { return value_->get<std::uint64_t>(); }
  double read_floating() const { return value_->get<double>(); }
  std::string_view read_string() const { return value_->get<std::string>(); }
  ...
  const serialization_value &value() const noexcept { return *value_; }
