...
template <typename T, std::size_t I> struct automatic_field {
  ...
  static constexpr std::string_view wire() { return member_name<I, T>(); }
  ...
  static consteval bool names_unique() { return true; }
  template <typename M> static M make_default() = delete;
};
template <typename T, std::size_t I, std::size_t J = 0>
consteval auto field_descriptor() {
...
using field_descriptor_t = decltype(field_descriptor<T, I>());
  ...
  [&]<std::size_t... I>(std::index_sequence<I...>) {
    auto one = [&]<std::size_t A>() {
      [&]<std::size_t... J>(std::index_sequence<J...>) {
        ((valid = valid &&
                  (std::tuple_element_t<A, decltype(fields)>::member
                       .to_string_view() !=
                   std::tuple_element_t<A + 1 + J, decltype(fields)>::member
                       .to_string_view())),
         ...);
      }
...
template <typename T> consteval bool schema_wire_names_unique() {
  ...
  [&]<std::size_t... I>(std::index_sequence<I...>) {
    auto one = [&]<std::size_t A>() {
      ...
      [&]<std::size_t... J>(std::index_sequence<J...>) {
        ((valid = valid &&
                  !DA::template overlaps<
                      field_descriptor_t<T, A + 1 + J>>()),
         ...);
      }(std::make_index_sequence<N - A - 1>{});
    };
    (one.template operator()<I>(), ...);
...
template <typename T> consteval bool valid_schema() {
  return schema_members_exist<T>() && schema_members_unique<T>() &&
         schema_wire_names_unique<T>() && schema_field_names_unique<T>();
}
template <typename T> consteval std::size_t serialized_member_count() {
  constexpr std::size_t N = member_count_v<T>;
  return []<std::size_t... I>(std::index_sequence<I...>) {
    return (std::size_t{0} + ... +
            (field_descriptor_t<T, I>::is_transient ? 0u : 1u));
  }(std::make_index_sequence<N>{});
}
} // namespace gmp::detail
#endif
